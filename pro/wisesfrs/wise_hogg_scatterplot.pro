...
;-
pro wise_hogg_scatterplot, xxx,yyy,weight=weight, $
  ...
  cannotation=cannotation,$
                      ...
                      usegrid=usegrid, $
  overplot=overplot, noredraw=noredraw, $
  frac_outliers=frac_outliers,$
  smooth_contours=smooth_contours, $
  log=log, keynote=keynote, $ ; hack for MF completeness plots!
...
deltay= (yrange[1]-yrange[0])/double(ynpix)
...
; hack for mfplot_completeness! get the number of points in each bin
...
; smooth the contours; should make the smoothing size an input keyword!
if (n_elements(smooth_contours) ne 0) then begin
   gsmooth = smooth_contours>1
   kernel = psf_gaussian(npixel=2*gsmooth,fwhm=gsmooth*[1,1],ndim=2,/norm)
   cumimage = convol(cumimage,kernel,/edge_truncate)
endif
...
; hack for the MF completeness plot!! make white any bin with no
; points (need to use DGRID here, not GRID!)
    zero = where(dgrid le 0.0,nzero)
;   zero = where(grid le 0.0,nzero)
;   zero = where(weight le 0.0,nzero)
    if (nzero ne 0L) then begin
       if keyword_set(keynote) then col = fsc_color('black',0) else col = fsc_color('white',0)
       tvgrid[zero] = 0
    endif    
...
; plot greyscale; re-draw the axes
    ...
    if (keyword_set(noredraw) eq 0) then begin
...
; otherwise overplot contours
    ...
    if (NOT keyword_set(nocontours)) then begin
        contour, cumimage,xvec,yvec,levels=levels,/overplot, $
          c_labels=lonarr(n_elements(levels))+labelcont,c_thick=cthick,c_color=ccolor,$
          c_annotation=cannotation
    endif
endelse
...
; plot a random FRAC_OUTLIERS subset of the outliers
            if (n_elements(frac_outliers) gt 0) then $
              these = long(randomu(seed,n_elements(ioutliers)*frac_outliers)*n_elements(ioutliers)) else $
                these = lindgen(n_elements(ioutliers))
            djs_oplot, x[ioutliers[these]], y[ioutliers[these]], $
